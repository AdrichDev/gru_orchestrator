...
export class RufloProvider implements GruProvider {
  ...
  async checkAvailability(): Promise<ProviderAvailability> {
    const probe = await probeCommand("pnpm", ["dlx", "ruflo@3.16.2", "--version"]);
    return {
      providerId: this.id,
      available: probe.available,
      executable: "pnpm dlx ruflo@3.16.2",
      version: probe.version,
      reason: probe.reason,
      installHint: "Ejecuta: pnpm dlx ruflo@3.16.2 init wizard"
    };
  }
  async run(task: ProviderTask): Promise<ProviderResult> {
    try {
      ...
      const result = await execa(
        "pnpm",
        ["dlx", "ruflo@3.16.2", "workflow", "run", "-t", "development", "--task", task.prompt],
        { reject: false }
      );
      return {
        providerId: this.id,
        ...
        executedCommand: `pnpm dlx ruflo@3.16.2 workflow run -t development --task ${JSON.stringify(task.prompt)}`
      };
    } catch (error) {
